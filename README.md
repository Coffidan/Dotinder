...
2. Сервис уведомлений.Подтверждение регистрации (email/Steam).Оповещения о мэтчах (когда кто-то лайкнул ваш профиль). Уведомления о новых сообщениях в чате. Напоминания (если игрок долго не отвечает на запрос). Рассылка обновлений (новые функции, турниры на платформе).

3. Основной сервис (поиск и взаимодействие).Профиль игрока: MMR, предпочитаемые роли, герои, Winrate. Ссылки на Dotabuff/Steam. Отзывы от других игроков (чтобы избегать токсиков). Система свайпов (Tinder-механика): Лайк/дизлайк анкет других игроков. Мэтч при взаимном интересе → открывается чат. Расширенный поиск: Фильтры по MMR, ролям, языку, режимам (ранкед, турниры, казуал). Поиск по гибким критериям (например, "ищу саппорта для спам-комбо с Яттом"). Лобби и чаты: Создание групп для стека. Турнирные комнаты (для организаторов).
...
v1.1:
  ...
  - Уведомления
    - О мэтчах, новых сообщениях
    - Email/WebPush-оповещения
...
title Диграмма последовательности проекта Дотиндер
Пользователь → Сервис аутентификации: Запрос на регистрацию через Steam
Сервис аутентификации → Steam API: Проверка валидности Steam ID
Steam API → Сервис аутентификации: Подтверждение данных игрока (MMR, никнейм)
Сервис аутентификации → База данных: Сохранение нового пользователя
Сервис аутентификации → Сервис уведомлений: Запрос на отправку приветственного email (если указан)
Сервис уведомлений → Почта пользователя: Письмо с подтверждением регистрации
Пользователь → Почта пользователя: Переход по ссылке подтверждения
Почта пользователя → Сервис аутентификации: Подтверждение верификации email
Сервис аутентификации → Пользователь: Успешная авторизация 
